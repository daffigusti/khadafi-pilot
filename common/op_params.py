...
from openpilot.common.colors import COLORS
from openpilot.common.travis_checker import BASEDIR
...
class opParams:
  def __init__(self, calling_function=''):
    ...
    self.fork_params = {'camera_offset': Param(-0.04 if TICI else 0.06, NUMBER, 'Your camera offset to use in lane_planner.py\n'
                        ...
                        'TUNE_LAT_PID_kd': Param([0.0, 0.0], [list, float], kd_desc + 'This scales the low-speed response.\n', live=True, min_val=0.0, max_val=10.0, linked_op_param_check_param='TUNE_LAT_PID_link_ls_hs', show_op_param='TUNE_LAT_type', show_op_param_check_val='pid'),
                        ...
                        'TuneTorqueParam': Param(False, bool, '', live=True),
                        'LiveTuneTorque': Param(False, bool, '', live=True),
                        'TORQUE_LAT_ACCEL': Param(1.8, NUMBER, '', live=True),
                        'TORQUE_FRICTION': Param(0.1, NUMBER, '', live=True),
                        
                        'OBDDataNum': Param(0, NUMBER, '', live=True),
                        'OBDDataSoc': Param(0, NUMBER, '', live=True),
                        'OBDDataChargeTimes': Param(0, NUMBER, '', live=True),
                        'OBDDataSoh': Param(0, NUMBER, '', live=True),
