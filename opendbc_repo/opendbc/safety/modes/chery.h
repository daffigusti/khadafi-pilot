...
static void chery_rx_hook(const CANPacket_t *to_push)
{
  ...
  if (bus == CHERY_MAIN)
  {
    ...
    // DBC: SG_ WHEEL_SPEED_FR : 7|16@0- (0.00829,0) - Front Right (Motorola big-endian)
    //      SG_ WHEEL_SPEED_FL : 23|16@0- (0.00829,0) - Front Left (Motorola big-endian)
    if (addr == CHERY_WHEEL_SENSOR)
    {
      // Motorola big-endian 16-bit: start bit 7 means byte0=bits[15:8], byte1=bits[7:0]
      int16_t front_right_raw = (int16_t)(((int)to_push->data[0] << 8) | to_push->data[1]);
      // Motorola big-endian 16-bit: start bit 23 means byte2=bits[15:8], byte3=bits[7:0]
      int16_t front_left_raw = (int16_t)(((int)to_push->data[2] << 8) | to_push->data[3]);
    ...
    // DBC: SG_ TORQUE_DRIVER : 7|12@0- (0.24,0) - 12-bit signed, Motorola big-endian
    // Note: carstate.py multiplies by direction for sign handling
    if (addr == CHERY_STEER_SENSOR_2)
    {
      // Motorola big-endian 12-bit: start bit 7 means byte0=bits[11:4], byte1[7:4]=bits[3:0]
      int torque_raw = (((int)to_push->data[0] << 4) | (to_push->data[1] >> 4)) & 0xFFF;
      // Sign extend from 12-bit to full int width
      if (torque_raw & 0x800)
      {                          // If bit 11 is set (negative)
        torque_raw |= ~0xFFF;    // Sign extend to full int
      }
    ...
    // DBC: SG_ STEER_ANGLE : 7|14@0+ (0.1,-780) - 14-bit unsigned, Motorola big-endian
    if (addr == CHERY_STEER_ANGLE_SENSOR)
    {
      // Motorola big-endian 14-bit: start bit 7 means byte0=bits[13:6], byte1[7:2]=bits[5:0]
      int angle_raw = (((int)to_push->data[0] << 6) | (to_push->data[1] >> 2)) & 0x3FFF;
      // Apply scale (0.1) and offset (-780): angle_deg = raw * 0.1 - 780
      // Convert to deg*100 units (matching desired_angle scale): angle_deg * 100 = (raw - 7800) * 10
      int angle_meas_new = (angle_raw - 7800) * 10; // Units: deg * 100 (matches desired_angle)
  ...
  else if (bus == CHERY_CAM)
  {
    ...
    {
      // ACC_STATE : 9|2@0+ (Motorola, byte 1 bits 1:0)
      // 1 = NOT AVAILABLE, 2 = AVAILABLE, 3 = ACTIVE
      acc_main_on = ((GET_BYTES(to_push, 1, 1) & 0x03) != 1U);

      // GAS_PRESSED : 47|1@0+ (Motorola, byte 5 bit 7)
      gas_pressed = (GET_BYTES(to_push, 5, 1) & 0x80U) != 0U;
    }
    if (addr == CHERY_ACC_DATA)
    {
      // ACC_ACTIVE : 20|1@0+ (Motorola, byte 2 bit 4)
      // Combine cruise_engaged with acc_main_on so that the rising-edge detection
      // in pcm_cruise_check() fires only when BOTH are true.
      // This avoids a race condition where ACC arrives before ACC_CMD in the
      // first cycle, burning the rising edge while acc_main_on is still false.
      bool cruise_engaged = GET_BIT(to_push, 20U);
      pcm_cruise_check(cruise_engaged && acc_main_on);
    }
  }

  // Safety: Disengage controls on brake or gas press
  // This provides a critical safety fallback for driver override
  if (brake_pressed || gas_pressed)
  {
    controls_allowed = false;
  }
  // Note: acc_main_on is now embedded in pcm_cruise_check() above via
  // pcm_cruise_check(cruise_engaged && acc_main_on), so a separate
  // !acc_main_on check here is not needed and would cause the race condition.
}
...
static bool chery_tx_hook(const CANPacket_t *to_send)
{
  ...
  if ((bus == CHERY_MAIN) && (addr == CHERY_LKAS_CMD))
  {
    ...
    // DBC: SG_ CMD : 6|13@0- (1,0) - Motorola big-endian, 13-bit signed, start bit 6 (MSB)
    ...
    // Motorola big-endian 13-bit: start bit 6 means byte0[6:0]=bits[12:6], byte1[7:2]=bits[5:0]
    int16_t can_angle_raw = (int16_t)((((int16_t)(to_send->data[0] & 0x7F) << 6) | (to_send->data[1] >> 2)) & 0x1FFF);
    // Sign extend from 13-bit to 16-bit
    if (can_angle_raw & 0x1000)
    {
      can_angle_raw |= (int16_t)0xE000; // Set upper bits to 1 for negative values
    }
  ...
  if (chery_longitudinal && (bus == CHERY_MAIN) && (addr == CHERY_ACC_CMD))
  {
    ...
    // DBC: SG_ CMD : 6|10@0- (1,0) - Motorola big-endian, 10-bit signed, start bit 6 (MSB)
    // Motorola: byte0[6:0]=bits[9:3], byte1[7:5]=bits[2:0]
    int16_t cmd_raw = (int16_t)((((int16_t)(to_send->data[0] & 0x7F) << 3) | (to_send->data[1] >> 5)) & 0x3FF);
    // Sign extend from 10-bit to 16-bit
    if (cmd_raw & 0x200)
    {                    // If bit 9 is set (negative)
      cmd_raw |= (int16_t)0xFC00; // Set upper 6 bits
    }
