...
ENABLE_BUTTONS = (CruiseButtons.RES_ACCEL, CruiseButtons.DECEL_SET, CruiseButtons.MAIN)

BUTTONS_DICT = {CruiseButtons.RES_ACCEL: ButtonType.accelCruise, 
                CruiseButtons.DECEL_SET: ButtonType.decelCruise,
                CruiseButtons.MAIN: ButtonType.setCruise,
                CruiseButtons.GAP_UP: ButtonType.gapAdjustCruise, 
                CruiseButtons.GAP_DOWN: ButtonType.gapAdjustCruise,
...
class CarInterface(CarInterfaceBase):
  ...
  def _get_params(ret, candidate, fingerprint, car_fw, experimental_long, docs):
    ...
    ret.pcmCruise = True
    ret.enableBsm = 0xb1 in fingerprint[0]  # BSM
  ...
  def _update(self, c):
    ...
    allow_enable = self.CS.cruise_buttons == CruiseButtons.MAIN

    # The ECM allows enabling on falling edge of set, but only rising edge of resume
    events = self.create_common_events(ret, c, extra_gears=[GearShifter.sport, GearShifter.low,
                                                         GearShifter.eco, GearShifter.manumatic], pcm_enable=False, allow_enable=allow_enable)
    #if not self.CP.pcmCruise:
    #  if any(b.type == ButtonType.accelCruise and b.pressed for b in ret.buttonEvents):
    #    events.add(EventName.buttonEnable)

    events, ret = self.create_sp_events(self.CS, ret, events, enable_pressed=self.CS.accEnabled, allow_enable=allow_enable)
