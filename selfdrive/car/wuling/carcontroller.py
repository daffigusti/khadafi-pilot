...
class CarController:
  ...
  def update(self, CC, CS, now_nanos):
    ...
    if hud_v_cruise > 70:
      hud_v_cruise = 0
    set_speed_in_units = hud_v_cruise * (CV.MS_TO_KPH if CS.is_metric else CV.MS_TO_MPH)
   # show LFA "white_wheel" and LKAS "White car + lanes" when not CC.latActive
    ...
    if CC.cruiseControl.cancel:
      ...
      if (CS.resume_alert == 1 or CC.cruiseControl.resume) and self.frame % 2 == 0:
        ...
        # Send Resume button when planner wants car to move
          can_sends.extend([wulingcan.create_buttons(self.packer_pt, CS.buttons_counter, CruiseButtons.RES_ACCEL)] * 5)
          self.last_button_frame = self.frame
      elif CS.out.cruiseState.enabled and not self.CP.pcmCruiseSpeed:
        self.cruise_button = self.get_cruise_buttons(CS, CC.vCruise)
        if self.cruise_button is not None and CS.cruise_buttons == CruiseButtons.UNPRESS:
          # if self.frame % 2 == 0:
            print(self.cruise_button)
            can_sends.extend([wulingcan.create_buttons(self.packer_pt, CS.buttons_counter, self.cruise_button)] * 10)
    ...
    if self.frame % 2 == 0:
      lead_distance = 0
      # test = CS.lkas_enabled
      test = CS.gap_dist_button == 5
      if test:
        self.acc = self.prev_acc
        print('enable')
      else:
        self.acc = CS.acc_cmd
        self.prev_acc = CS.acc_cmd
      ...
      can_sends.extend(wulingcan.create_radar_command(self.packer_pt, CS.acc_cmd, self.acc, test, self.frame, CC, CS))
      # if CS.out.steeringPressed:
